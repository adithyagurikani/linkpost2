export function parseSourceContent(source: {
  ...
  if (source.sourceType === "json") {
    try {
      ...
      const contents = items.map((i: unknown) => {
        if (typeof i === "string") return i;
        return String((i as Record<string, unknown>).content || (i as Record<string, unknown>).text || JSON.stringify(i));
      }).filter(Boolean);
      return { contents };
    } catch {
...
function parseCSVLine(line: string): string[] {
  ...
  if (source.sourceType === "csv") {
    const contents = lines.map((l: string) => {
      const trimmed = l.trim();
      if (!trimmed || trimmed.startsWith("#")) return "";
      const fields = parseCSVLine(trimmed);
      return fields.length > 1 ? fields.slice(1).join(",").trim() : fields[0].trim();
    }).filter(Boolean);
    return { contents };
  }
